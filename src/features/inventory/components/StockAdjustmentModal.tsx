...
const StockAdjustmentModal: React.FC<StockAdjustmentModalProps> = ({ item, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center sm:p-4 bg-slate-900/40 backdrop-blur-sm animate-fade-in">
      <div className="bg-white w-full max-w-md rounded-t-[32px] sm:rounded-[24px] shadow-2xl overflow-hidden animate-slide-up">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
          ...
          <div className="pt-2 pb-safe flex gap-3">
            ...
            <Button type="submit" className="flex-1 rounded-xl h-12 font-bold bg-slate-900 hover:bg-slate-800 text-white" loading={isSubmitting}>
              {t('common.save', 'Confirm Adjustment')}
            </Button>
          </div>
        </form>
      </div>
