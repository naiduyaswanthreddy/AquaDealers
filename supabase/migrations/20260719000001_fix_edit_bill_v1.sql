-- Keep bill edits in-place and reconcile inventory / farmer due atomically.
-- This replaces the original edit RPC, which applied the farmer delta using
-- the invoice total instead of the outstanding balance and altered stock on
-- historical bills that were never deducted from inventory.
    ...
    IF NOT COALESCE(v_bill.is_historical, false)
      ...
      ELSE
        ...
        LOOP
          ...
          UPDATE public.bill_item_lot_allocations
          SET quantity = quantity + v_take, unit_price = v_new_unit_price
