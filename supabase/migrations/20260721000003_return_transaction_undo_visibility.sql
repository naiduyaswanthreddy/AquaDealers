-- Return reversals already have an atomic undo handler. Expose them in the
-- transaction feed during the same undo window as other financial events.
...
BEGIN
  ...
  WHERE e.dealer_id = v_dealer_id
    ...
    AND (p_search IS NULL OR e.reference ILIKE '%' || p_search || '%' OR e.description ILIKE '%' || p_search || '%');
  ...
  FROM (
    SELECT e.*, b.name AS branch_name,
      (e.status = 'active' AND e.undo_expires_at > now()
       AND e.source_type IN ('bill', 'bill_return', 'farmer_payment', 'stock_purchase', 'supplier_payment', 'stock_transfer', 'expense', 'cash_entry')) AS can_undo,
      CASE WHEN e.status = 'undone' THEN 'Undone'
        WHEN e.status = 'cancelled' THEN 'Cancelled'
        WHEN e.undo_expires_at <= now() THEN 'Finalized'
