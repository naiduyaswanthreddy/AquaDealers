-- =============================================================================
-- bill_v2_idempotency_key — 2026-09-17
--
-- Root cause fix for duplicate bills: bills.idempotency_key (added in
-- 20260715000002_scalability_fixes.sql, with a partial unique index on
-- (dealer_id, idempotency_key)) was never written or checked by
-- create_bill_v2, so it was a no-op. The real failure sequence:
--   1. Dealer submits a bill. create_bill_v2 commits successfully.
--   2. The response is lost on a flaky connection (e.g. a poorly-connected
--      branch) -> the client sees a network error.
--   3. The client falls back to the offline queue with a *new* client_ref
--      (it has no way to know the original request already succeeded).
--   4. On reconnect, create_bill_offline_sync only dedupes by client_ref,
--      which is new, so it calls create_bill_v2 again -> second bill row,
--      stock decremented twice, farmer dues incremented twice.
--
-- Fix: create_bill_v2 now honors idempotency_key itself (the same value the
-- client already sends and already survives the offline-fallback path
-- unchanged). If a bill with the same (dealer_id, idempotency_key) already
-- exists, return it instead of inserting a new one. This closes the gap for
-- both the direct call and create_bill_offline_sync, since the latter just
-- wraps create_bill_v2 with the same payload.
-- =============================================================================
...
BEGIN
  ...
  -- Idempotency: if a bill with this key already exists for this dealer,
  ...
  INSERT INTO bills (
    ...
    v_is_estimate, v_idempotency_key
  )
